...
const WrapperAbsolute = styled.svg`
    ...
    ${PathAbsolute} {
        stroke: ${({ activePathItem, color }) =>
            activePathItem ? color : "transparent"};
    }

    ${(props) =>
        props.activePathItem === 1 &&
        css`
            ${PathAbsolute}.one {
                animation: ${circleStroke(1145, 600)} ${`${props.time / 1000}s`}
                    linear;
            }
            ${PathAbsolute}.two {
    ...
    ${(props) =>
        props.activePathItem === 2 &&
        css`
            ${PathAbsolute}.one {
                stroke-dashoffset: 600;
            }
            ${PathAbsolute}.two {
                animation: ${circleStroke(980, 543)} ${`${props.time / 1000}s`}
                    linear;
            }
            ${PathAbsolute}.three {
                stroke-dashoffset: 800;
  ...
  ${(props) =>
        ...
        css`
            ...
            ${PathAbsolute}.three {
                animation: ${circleStroke(1000, 1600)}
                    ${`${props.time / 1000}s`} linear;
            }
        `}
`
